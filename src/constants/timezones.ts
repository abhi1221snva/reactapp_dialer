...
const TIMEZONE_MAP = new Map(TIMEZONES.map(tz => [tz.value, tz.label]))

/** Look up the display label for a timezone IANA value. */
export function getTimezoneLabel(value: string | undefined | null): string {
  if (!value) return 'Eastern Time (US & Canada)'
  const label = TIMEZONE_MAP.get(value)
  if (label) {
    // Strip the "(GMT±XX:XX) " prefix for compact display
    return label.replace(/^\(GMT[^)]*\)\s*/, '')
  }
  // Fallback: show last segment of IANA string
  return value.split('/').pop()?.replace(/_/g, ' ') ?? value
}

/** Full label including GMT offset, for detailed views. */
export function getTimezoneFullLabel(value: string | undefined | null): string {
  if (!value) return '(GMT-05:00) Eastern Time (US & Canada)'
