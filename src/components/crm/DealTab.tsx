...
export function DealTab({ leadId }: Props) {
  ...
  return (
    <div className="space-y-5">
      {deal ? (
        <DealDetailsCard deal={deal} leadId={leadId} />
      ) : (
        <div className="bg-white border border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm font-semibold text-slate-500">No funded deal recorded yet</p>
          <p className="text-xs text-slate-400">Use the form below to record a deal when funding is confirmed.</p>
        </div>
      )}
