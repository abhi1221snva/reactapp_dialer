...
import { emailTemplateService, type EmailTemplatePayload } from '../../services/emailTemplate.service'
...
const PICKER_TIPS = [
  ...
  'Works in both subject & body fields',
]
...
const EMPTY_FORM: FormState = {
  template_name: '', subject: '', template_html: '', type: 'general',
}
...
function TemplateModal({
  ...
  editing?: DialerEmailTemplate | null
  onClose: () => void
  onSaved: () => void
}) {
  ...
  const [form, setForm] = useState<FormState>(
    editing
      ? { template_name: editing.template_name, subject: editing.subject ?? '', template_html: editing.template_html ?? '', type: editing.type ?? 'general' }
      : EMPTY_FORM
  )
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return isEdit
        ? emailTemplateService.update(editing!.id, payload)
        : emailTemplateService.create(payload)
    },
    onSuccess: () => { toast.success(isEdit ? 'Template updated' : 'Template created'); qc.invalidateQueries({ queryKey: ['dialer-email-templates'] }); onSaved() },
          ...
          <div className="px-6 pb-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select className="input w-full text-sm" value={form.type} onChange={e => set('type', e.target.value)}>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Hi [[first_name]], your update…"
                value={form.subject}
          ...
          {tab === 'edit' ? (
            <div className="flex flex-1 min-h-0 overflow-hidden">
              ...
              <div className="w-60 border-l border-slate-100 flex-shrink-0 bg-slate-50/60 flex flex-col min-h-0">
                <PlaceholderPicker placeholders={placeholders ?? []} loading={placeholdersLoading} onInsert={insertVar} tipLines={PICKER_TIPS} />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto bg-gradient-to-b from-slate-50 to-slate-100/80">
              <div className="max-w-2xl mx-auto py-8 px-5 space-y-4">
                <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-sky-200 bg-sky-50">
                  ...
                  <p className="text-xs text-sky-700">Preview mode — variables shown as raw placeholders</p>
                </div>
...
function ListItem({
  t, selected, onClick,
}: { t: DialerEmailTemplate; selected: boolean; onClick: () => void }) {
  const stripped = useMemo(() =>
    (t.template_html ?? '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 100),
    [t.template_html]
  )
  const isActive = String(t.status) === '1'
  return (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-1 mb-0.5">
            ...
            {t.type && t.type !== 'general' && (
              <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-sky-100 text-sky-600 uppercase tracking-wide flex-shrink-0">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 flex-shrink-0 bg-white">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0',
            ...
            <Mail size={14} className={isActive ? 'text-indigo-500' : 'text-slate-400'} />
          </div>
...
export function EmailTemplates() {
  ...
  const [editing, setEditing]         = useState<DialerEmailTemplate | null>(null)
  const [selected, setSelected]       = useState<DialerEmailTemplate | null>(null)
  const [search, setSearch]           = useState('')
  const [statusFilter, setStatusFilter] = useState<'all' | 'active' | 'inactive'>('all')

  const openCreate = () => { setEditing(null); setShowModal(true) }
  const openEdit   = (t: DialerEmailTemplate) => { setEditing(t); setShowModal(true) }
  const closeModal = () => { setShowModal(false); setEditing(null) }

  const { data: rawData, isLoading } = useQuery({
    queryKey: ['dialer-email-templates'],
    queryFn: async () => {
      const res = await emailTemplateService.list()
      // Backend returns { data: { data: [...], total, ... } } or { data: [...] }
      const inner = res.data?.data
      if (Array.isArray(inner)) return inner as DialerEmailTemplate[]
      if (inner?.data && Array.isArray(inner.data)) return inner.data as DialerEmailTemplate[]
      return [] as DialerEmailTemplate[]
    },
    staleTime: 0,
  })
  ...
  const toggleMutation = useMutation({
    mutationFn: (t: DialerEmailTemplate) => {
      const newStatus = String(t.status) === '1' ? 0 : 1
      return emailTemplateService.toggleStatus(t.id, newStatus)
    },
    onSuccess: () => { toast.success('Status updated'); qc.invalidateQueries({ queryKey: ['dialer-email-templates'] }) },
