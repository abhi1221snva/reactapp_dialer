...
export function EditRingless() {
  ...
  useEffect(() => {
    if (loaded || !campaignData) return
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const outer = (campaignData as any)?.data
    const nested = outer?.data ?? outer ?? {}
    // Show endpoint may return array — unwrap first element
    ...
    reset({
      ...
      call_ratio: String(raw.call_ratio ?? '1'),
      duration: String(raw.duration ?? raw.call_duration ?? ''),
      caller_id: raw.caller_id ?? 'area_code',
      custom_caller_id: String(raw.custom_caller_id ?? ''),
      country_code: String(raw.country_code ?? '1'),
      voice_template_id: String(raw.voice_template_id ?? ''),
      sip_gateway_id: raw.sip_gateway_id ? String(raw.sip_gateway_id) : '',
      time_based_calling: Number(raw.time_based_calling ?? 0),
      call_time_start: raw.call_time_start ?? '09:00',
      call_time_end: raw.call_time_end ?? '17:00',
      timezone: raw.timezone ?? 'America/New_York',
      timezone_rule: Number(raw.timezone_rule ?? 0),
    })
    // Restore call timer state from existing data
    if (raw.call_schedule_id) {
      setSelectedTimerKey(Number(raw.call_schedule_id))
    } else if (Number(raw.time_based_calling) === 1) {
      setSelectedTimerKey('custom')
    }
    setLoaded(true)
  }, [campaignData, loaded, reset])
  ...
  if (loadingCampaign) {
    return (
      <div className="-mx-5 -mt-5 flex flex-col items-center justify-center" style={{ height: 'calc(100vh - 70px)' }}>
        <Loader2 size={28} className="animate-spin text-blue-500" />
        <p className="text-sm text-slate-400 mt-3">Loading campaign…</p>
      </div>
    )
  }
    ...
    <div className="-mx-5 -mt-5 flex flex-col animate-fadeIn" style={{ height: 'calc(100vh - 70px)' }}>
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-slate-200 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[15px] font-semibold text-slate-800">Edit Ringless Campaign</h1>
        </div>
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => navigate('/ringless')}
            className="text-xs text-slate-400 hover:text-slate-600 font-medium transition-colors hidden sm:block">Cancel</button>
          <button type="submit" form="edit-ringless-form" disabled={isPending}
      ...
      {/* Form */}
      <form id="edit-ringless-form" onSubmit={handleSubmit(onSubmit, onFormError)} noValidate className="flex-1 overflow-hidden">
