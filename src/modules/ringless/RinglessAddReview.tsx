...
import {
  ArrowLeft, CheckCircle2, Voicemail, Phone, Clock, Mail, List,
  Globe, Users, PartyPopper,
} from 'lucide-react'
...
export function RinglessAddReview() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }} className="animate-fadeIn">
      ...
      {/* ── .lt Header Toolbar ── */}
      <div className="lt">
        <div className="lt-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button type="button" onClick={() => navigate(`/ringless/${campaignId}/attach-leads`)} className="lt-b" title="Back">
            <ArrowLeft size={13} />
          </button>
          <h1>Review Campaign</h1>
          {c.title && (
            <span className="lt-desc">{c.title} — #{campaignId}</span>
          )}
        </div>

        {/* Step Indicator */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, marginLeft: 4 }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#059669', fontWeight: 500 }}>
            <span style={{ width: 18, height: 18, borderRadius: '50%', background: '#d1fae5', color: '#047857', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700 }}>1</span>
            Details
            <CheckCircle2 size={12} style={{ color: '#10b981' }} />
          </span>
          <span style={{ width: 16, height: 1, background: '#e2e8f0' }} />
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#059669', fontWeight: 500 }}>
            <span style={{ width: 18, height: 18, borderRadius: '50%', background: '#d1fae5', color: '#047857', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700 }}>2</span>
            Leads
            <CheckCircle2 size={12} style={{ color: '#10b981' }} />
          </span>
          <span style={{ width: 16, height: 1, background: '#e2e8f0' }} />
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#3b82f6', fontWeight: 700 }}>
            <span style={{ width: 18, height: 18, borderRadius: '50%', background: '#3b82f6', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700 }}>3</span>
            Review
          </span>
        </div>

        <div className="lt-right">
          <button type="button" onClick={() => navigate('/ringless')} className="lt-b lt-p">
      ...
      {/* ── Page Content ── */}
      <div className="w-full space-y-5" style={{ marginTop: 8 }}>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {listsLoading ? (
          ...
          <div className="text-center py-12 px-6">
            ...
            <button type="button" onClick={() => navigate(`/ringless/${campaignId}/attach-leads`)}
      ...
      <div className="flex items-center justify-between pt-1 pb-4">
        ...
        <button type="button"
          onClick={() => navigate('/ringless')}
          className="btn-primary px-6">
          <PartyPopper size={15} />
          Done — Go to Campaigns
        </button>
      </div>
      </div>{/* close .w-full.space-y-5 wrapper */}
    </div>
  )
}
